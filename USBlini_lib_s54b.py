...
def printlog(msg, filename = "log.txt"):
    print(msg)
    try:
...
#     printlog(f'USBlini_process_no_{linino} PID is: {os.getpid()}')    
    ...
    try:
        ...
        printlog(msg)
    Error = False       #flag used to restart the process after XXs in case if sth goes wrong
    Error_time = 0      #time stamp when the error occured, used to calculate the duration of the error event
    Error_time_limit_s = 3     #after this time in s while Error == True, the process_error_event will be .set(), what will try to terminate and restart the process
                    ...
                    for frame_ts in frames_to_send:
                        ...
                        if status == 'received':
                            try:
                                ...
                                printlog(f'linino: {linino}, serial: {linis[linino]}, frame: {respo[1]}, dec.signals: {decoded_signals}')
#                                 printlog(shared_signals)
                                #printlog(decoded_signals)
                            except:
...
#                                 shared_signals = decoded_signals
                ...
                try:
                    ...
                    printlog(f'try to initialize linino: {linino}, serial: {linis[linino]}')
                except:
                    printlog(f'process fatal error on linino: {linino}, serial: {linis[linino]}')
                    process_error_event[linino].set()          
        else:
            ct = datetime.datetime.now()
            with open(temp_filepath, 'w') as plik:
...
# ========= FOLDER SETUP CODE ===================
    ...
    for nazwa_folderu in required_folders:
        if os.path.exists(nazwa_folderu):
            printlog(f'Wymagany folder "{nazwa_folderu}" istnieje.')
        else:
            os.mkdir(nazwa_folderu)
            if os.path.exists(nazwa_folderu):
                printlog(f'Folder "{nazwa_folderu}" został utworzony pomyślnie.')
            else:
                printlog(f'Nie udało się utworzyć folderu "{nazwa_folderu}"')
...
#         printlog('START was pressed!!!')
        global running
        running = True

    def stop():
        """Stop scanning by setting the global flag to False."""
#         printlog('STOP was pressed!!!')
        global running
        running = False
...
#         printlog('.')
        time.sleep(0.1)
    printlog('.')
    printlog(f'\'running\' is set to: {running}')
    ...
    while running != None:
        ...
        if not log_flag_process.is_alive():
            try:
                ...
                printlog(f'failed to terminate log flag process')
            try:
                log_flag_process_args = (log_interval, process_log_flags)
                log_flag_process = multiprocessing.Process(target=log_flag, args=log_flag_process_args)
                log_flag_process.start()
            except:
                printlog(f'failed to start log flag process')
        for linino in processes.keys():
            if process_error_event[linino].is_set() and processes[linino].is_alive():
                try:
                    processes[linino].terminate()
                except:
                    printlog(f'failed to terminate process of linino {linino} PID: {processes[linino].pid}')
            if not processes[linino].is_alive():
                ...
                processes[linino] = process
                processes[linino].start()
                printlog(f'process of linino {linino} revived, new PID: {processes[linino].pid}')
            try:
                temp_filepath = f'temp_files/temp_{resultFileNames[linino]}'
                with open(temp_filepath, 'r') as plik:
                    ...
                    string = plik.read()
                    string = string.strip()
#                     printlog(f'LIN {linino}, PID {processes[linino].pid}: {string}')
                if len(string) > 0:
